...
public class Boss2Script : MonoBehaviour {
    ...
    void IsDead()
    {
        ...
        if (animEnemic.GetCurrentAnimatorStateInfo(0).IsName("Boss2-Dead"))
        {
            animEnemic.SetBool("doDead", false);
            PlayerPrefs.SetString("level3FActive", "true");
            Invoke("GoToMenu", 6);

            Social.ReportProgress("CggIv9GCrUcQAhAB", 100.0f, (bool success) => {
    ...
    void PlayerNearEnimie()
    {
        ...
        if (diferencePositions >= -12 && diferencePositions <= -2.6f)
        {
            if (enemicScale.x < 0)
            {
        ...
        else if (diferencePositions <= 12 && diferencePositions >= 2.6f)
        {
            if (enemicScale.x > 0)
            {
        ...
        else if (diferencePositions >= -2.6f && diferencePositions <= 2.6f)
        {
            ...
            if (animEnemic.GetCurrentAnimatorStateInfo(0).IsName("Boss2-Idle") && animEnemic.GetCurrentAnimatorStateInfo(0).normalizedTime >= animEnemic.GetCurrentAnimatorStateInfo(0).length)
            {
